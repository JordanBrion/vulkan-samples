...
fn main() {
    unsafe {
        ...
        std::fs::File::open(
            // TODO spv vertex shader
        "/home/jordanbrion/Documents/rust/vk_001_compute_pipeline/shaders/002_compute_pipeline_2_buffers.comp.spv",
    )
    .expect("Something went wrong when opening vertex shader"),
            std::fs::File::open(
                // TODO spv fragment shader
        "/home/jordanbrion/Documents/rust/vk_001_compute_pipeline/shaders/002_compute_pipeline_2_buffers.comp.spv",
    )
    .expect("Something went wrong when opening fragment shader"),
    ];
        let v_shader_instructions = [
            ash::util::read_spv(&mut shader_files[0])
                .expect("Failed to read vertex shader spv file"),
            ash::util::read_spv(&mut shader_files[1])
                .expect("Failed to read fragment shader spv file"),
        ];
        let v_shader_module_create_infos = [
            ash::vk::ShaderModuleCreateInfo::builder().code(v_shader_instructions[0].as_slice()),
            ash::vk::ShaderModuleCreateInfo::builder().code(v_shader_instructions[1].as_slice()),
        ];
        ...
        let v_pipeline_shader_stage_create_infos = [
            ash::vk::PipelineShaderStageCreateInfo {
                ...
                module: logical_device
                    .create_shader_module(&v_shader_module_create_infos[0], None)
                    .expect("Cannot create shader module for vertex shader"),
            ...
            ash::vk::PipelineShaderStageCreateInfo {
                ...
                module: logical_device
                    .create_shader_module(&v_shader_module_create_infos[0], None)
                    .expect("Cannot create shader module for fragment shader"),
                p_name: shader_entry_name.as_ptr(),
                p_specialization_info: std::ptr::null(),
            },
        ];
...
p_scissors: std::ptr::null()
        ...
        let rasterization_state_create_info = ash::vk::PipelineRasterizationStateCreateInfo {
            ...
            p_next: std::ptr::null(),
            flags: Default::default(),
            depth_clamp_enable: true as Bool32,
            rasterizer_discard_enable: true as Bool32,
